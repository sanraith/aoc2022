...
const SOLUTIONS_DIRECTORY: &'static str = "src/solutions/";
const SOLUTIONS_MODULE_PATH: &'static str = "src/solutions.rs";
const MODULE_TEMPLATE_PATH: &'static str = "templates/solutions.rs.template";

fn main() {
    println!("cargo:rerun-if-changed={SOLUTIONS_DIRECTORY}");
...
fn generate_module() -> Result<(), Box<dyn Error>> {
    ...
    for entry in fs::read_dir(SOLUTIONS_DIRECTORY)? {
        let entry = entry?;
        ...
        let contents = fs::read_to_string(format!("{}/{}", SOLUTIONS_DIRECTORY, &file_name))?;
    ...
    let output = fs::read_to_string(MODULE_TEMPLATE_PATH)?;
    ...
    let mut file = std::fs::File::create(SOLUTIONS_MODULE_PATH)?;
